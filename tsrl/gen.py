        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
            ...
            for j in range(batch_x.shape[0]):
                ...
                if self.args.use_y:
                    ...
                    if self.args.norm:
                        input_query= instructions.use_norm_use_y.format(history_len=self.args.seq_len, 
                                                                    future_len=self.args.pred_len, 
                                                                    history=x_string,
                                                                    gt=y_string)
                    else:
                        input_query= instructions.use_y.format(history_len=self.args.seq_len, 
                                                                    future_len=self.args.pred_len, 
                                                                    history=x_string,
                                                                    gt=y_string)
                else:
                    ...
                    if self.args.norm:
                        input_query= instructions.use_norm.format(history_len=self.args.seq_len, 
                                                                    future_len=self.args.pred_len, 
                                                                    history=x_string)
                    else:
                        input_query = instructions.basic_inp.format(history_len=self.args.seq_len, 
                                                                    future_len=self.args.pred_len, 
                                                                    history=x_string)
                        
                tasks.append(async_call_llm(sys_prompt, input_query))
